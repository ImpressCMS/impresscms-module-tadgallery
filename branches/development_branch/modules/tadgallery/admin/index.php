...
//		//$('#div_form_'+div_sn).html(\"<form action=\'index.php\' method=\'post\'><table class=\'form_tbl\'><tr><td class=\'col\'><input type=\'text\' name=\'title\' value=\'{$title}\' style=\'width:130px;\'></td></tr><tr><td class=\'col\'><textarea style=\'width: 130px; height: 60px;\' name=\'description\'>{$description}</textarea></td></tr><tr><td colspan=2><input type=\'hidden\' name=\'sn\' value=\'\"+div_sn+\"\'><input type=\'hidden\' name=\'op\' value=\'update_tad_gallery\'><input type=\'submit\' value=\'�x�s\'></td></tr></table></form>\");
//impresscms loads jquery so we do not need it here
	$data="
	<script type='text/javascript'>
	function show_form(div_sn){
		$('#kasten'+div_sn).load('form.php?csn={$_GET['csn']}&sn='+div_sn).dialog({autoOpen: true, modal:true, title: 'Edit image'});
		return false;
	}
	...
	<table><tr><td valign='top'>
		...
		while(list($sn,$csn,$title,$filename,$size,$width,$height,$dir,$uid,$post_date,$counter,$good)=$xoopsDB->fetchRow($result)){
		  $good_btn=($good=='1')?"<a href='{$_SERVER['PHP_SELF']}?op=good_del&sn={$sn}&csn={$_GET['csn']}'><img src='".XOOPS_URL."/modules/{$MDIR}/images/good_del.png' alt='good_del.png, 1.1kB' title='Good del' border='0' height='22' width='22' hspace=2></a>":"<a href='{$_SERVER['PHP_SELF']}?op=good&sn={$sn}&csn={$_GET['csn']}'><img src='".XOOPS_URL."/modules/{$MDIR}/images/good_add.png' alt='good_add.png, 4.0kB' title='Good add' border='0' height='22' width='22' hspace=2></a>";
			...
			$fun=($show_function)?"
			<div id='kasten$sn' style='position: relative'></div>
			<img src='".XOOPS_URL."/modules/{$MDIR}/images/edit.gif' alt='"._BP_EDIT."' onClick=\"show_form('{$sn}'); return false;\">
