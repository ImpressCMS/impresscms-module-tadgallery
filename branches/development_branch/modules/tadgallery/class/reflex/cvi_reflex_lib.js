/**
 * cvi_reflex_lib.js 1.1 (16-Jan-2008)
			...
			if(document.all && document.namespaces && !window.opera) {
				...
				if(canvas.tagName.toUpperCase() == "CANVAS" && canvas.getContext("2d")) {
					...
					img.onload = function() {
						...
						context.save();
						context.translate(0,boxh-(canvas.isWK?0:1));
						context.scale(1,-1);
						...
						if(ibord>0) {
							...
							context.beginPath();
							tmp = 0; if(canvas.isOP&&itilt!='n') tmp = 1; if(canvas.isOP&&itilt=='n') tmp = -1;
							context.rect(bord,boxh-iheit+bord+tmp,ww-ibord,iheit);
							context.closePath();
							context.stroke();
							context.clearRect(0,boxh,ww,ibord);
						}
						if(!canvas.isWK||itilt=='n') {
							tmp = 0; if(canvas.isOP) tmp = 1;
							context.globalCompositeOperation = "destination-out";
							style = context.createLinearGradient(0,boxh-iheit-tmp,0,boxh);
						...
						if(canvas.isWK) {
							...
							tmp = 0; if(canvas.isOP&&itilt=='n') tmp = 1;
							context.fillRect(0,boxh-iheit-tmp,ww,iheit+tmp);
						}
						context.globalCompositeOperation = "source-over";
							...
							if(resource.getContext) {
								...
								if(canvas.getContext) {
									...
									if(itilt=='r') {
										...
										clipPolyLeft(context,ww/20,0,ww*0.95,boxh,iter+(rest>0?1:0),idist,iheit);
									}
									context.clip();
