...
define("_TADGAL_UP_FILE_DIR",ICMS_ROOT_PATH."/uploads/tadgallery/");
define("_TADGAL_UP_FILE_URL",ICMS_URL."/uploads/tadgallery/");
...
function mk_htaccess(){
  ...
  $main="SetEnvIfNoCase Referer \"^".ICMS_URL."(/|$)\" allowed=1
";
...
<rss version=\"2.0\" xmlns:media=\"http://search.yahoo.com/mrss\" xmlns:atom=\"http://www.w3.org/2005/Atom\">
	...
	<link>".ICMS_URL."/modules/tadgallery</link>
  ...
  while(list($sn,$csn,$title,$description,$filename,$size,$dir)=$xoopsDB->fetchRow($result)){
    ...
    $main.="		<item>
			<title>{$title}</title>
			<link>".ICMS_URL."/modules/tadgallery/view.php?sn={$sn}</link>
			<guid>{$sn}-{$csn}</guid>
...
function add_pic_mark($filename="",$newname="",$type="image/jpeg",$mark=""){
    ...
    $content=$mark;
		...
		//Ã¸»s¥D¼ÐÃD¤å¦r
		$font=ICMS_ROOT_PATH."/modules/pollution/wt011.ttf";
...
global $xoopsModule,$xoopsModuleConfig,$xoopsConfig;
	...
	if(is_array($adminmenu)){
		foreach($adminmenu AS $m){
			$td.="<a href='".ICMS_URL."/modules/{$dirname}/{$m['link']}'>{$m['title']}</a>";
		}
	}else{
		$td="<td></td>";
	}
	...
	#admtool a:link, #admtool a:visited {
		font-size: 12px;
		background-image: url(".ICMS_URL."/modules/{$dirname}/images/bbg.jpg);
		margin-right: 0px;
	...
	#admtool a:hover {
		background-image: url(".ICMS_URL."/modules/{$dirname}/images/bbg2.jpg);
		color: rgb(255,0,128);
	...
	<div id='admtool'>{$td}<a href='".ICMS_URL."/modules/{$dirname}/'>"._BACK_MODULES_PAGE."</a>
	</div>";
	return $main;
}
...
function toolbar($interface_menu=array()){
	...
	if ( $xoopsUser) {
		if ($moduleperm_handler->checkRight( 'module_admin', $xoopsModule->getVar( 'mid' ), $xoopsUser->getGroups() ) ) {
			$admin_tools="<a href='".ICMS_URL."/modules/{$dirname}/admin/index.php'>"._TO_ADMIN_PAGE."</a>";
		}
	}
	if(is_array($interface_menu)){
		foreach($interface_menu AS $title => $url){
			$td.="<a href='".ICMS_URL."/modules/{$dirname}/{$url}'>{$title}</a>";
		}
	}else{
		return;
	}
...
class PageBar{
	...
	function PageBar($total, $limit, $page_limit){
		...
		$this->prev = "<img src='".ICMS_URL."/modules/{$mydirname}/images/1leftarrow.gif' alt='"._BP_BACK_PAGE."' align='absmiddle' hspace=3>"._BP_BACK_PAGE;
		$this->next = "<img src='".ICMS_URL."/modules/{$mydirname}/images/1rightarrow.gif' alt='"._BP_NEXT_PAGE."' align='absmiddle' hspace=3>"._BP_NEXT_PAGE;
		$this->first = "<img src='".ICMS_URL."/modules/{$mydirname}/images/2leftarrow.gif' alt='"._BP_FIRST_PAGE."' align='absmiddle' hspace=3>"._BP_FIRST_PAGE;
		$this->last = "<img src='".ICMS_URL."/modules/{$mydirname}/images/2rightarrow.gif' alt='"._BP_LAST_PAGE."' align='absmiddle' hspace=3>"._BP_LAST_PAGE;
