...
function tadgallery_list($options){
    ...
    $description=Only1br($description);
	  $show_title=(empty($title))?"":"onmouseover=\"showToolTip(event,'<strong>{$title}</strong><br />{$description}');return false\" onmouseout=\"hideToolTip()\"";
	  ...
	  $pic_txt=($options[8]=='1')?"<div style='width:{$options[5]}px;{$options[9]}'>$txt</div>":"";
		...
		$pics.="
		<a href='".ICMS_URL."/modules/tadgallery/view.php?sn={$sn}' style='display:block;float:left;margin:{$options[4]}px;text-decoration:none;'><div style='border:1px solid #D0D0D0;width:{$options[5]}px;height:{$options[6]}px;background-image:url({$pic_url});background-position: top center;	background-repeat: no-repeat;cursor:pointer;' $show_title></div>$pic_txt</a>
		";
	}
