  ...
  $xoopsModule = &$modhandler->getByDirname("tadgallery");
	...
	while(list($com_id,$txt,$nsn,$uid,$title,$filename,$poster_uid)=$xoopsDB->fetchRow($result)){
		...
		if($options[2]=="1"){
			...
			$who="<div style='margin-bottom:6px;font-size:11px;width:{$options[1]}px;height:14px;overflow:hidden;'><a href='".ICMS_URL."/modules/tadgallery/author.php?uid=$poster_uid'>{$poster_uid_name}</a><img src='".ICMS_URL."/modules/tadgallery/images/image.png' hspace='4' align='absmiddle'><a href='".ICMS_URL."/modules/tadgallery/view.php?sn={$nsn}'>{$title}</a></div>";
		}else{
			$who="";
		}
		...
		$block.="<div style='width:{$options[1]}px;$css'><a href='".ICMS_URL."/userinfo.php?uid={$uid}'>{$uid_name}</a>: <a href='".ICMS_URL."/modules/tadgallery/view.php?sn={$nsn}#comment{$com_id}'>$txt</a></div>$who";
	}
	$block.="</div>";
	return $block;
}
