...
if($xoopsModuleConfig['only_thumb']!='1'){
	...
	while(list($fcsn,$title,$passwd,$show_mode,$cover)=$xoopsDB->fetchRow($result)){
	  ...
	  if($show_mode=="3d"){
		...
		$cover_pic=(empty($cover))?"images/folder_picture.png":ICMS_URL."/uploads/tadgallery/{$cover}";
...
//define("_BP_TOOLBAR","A total of %s pages, currently in section %s page %s");
$bar= sprintf(_BP_TOOLBAR,$mybar['total'],$mybar['current'], $mybar['current'])."{$mybar['left']}{$mybar['center']}{$mybar['right']}";
...
/*
		Tiny Bug where the images do not get showed
...
if($thumbnail_mode=="edge"){
	...
	mask2load[0] = '".ICMS_URL."/modules/{$MDIR}/class/edge/masks/8bit/crippleedge.png';
	mask2load[1] = '".ICMS_URL."/modules/{$MDIR}/class/edge/masks/8bit/frizzedge.png';
	mask2load[2] = '".ICMS_URL."/modules/{$MDIR}/class/edge/masks/8bit/softedge.png';
	// if you want IE to use mask images: add GIF versions
	// of the same name to the array (edge.js will find them)
	mask2load[3] = '".ICMS_URL."/modules/{$MDIR}/class/edge/masks/2bit/crippleedge.gif';
	mask2load[4] = '".ICMS_URL."/modules/{$MDIR}/class/edge/masks/2bit/frizzedge.gif';
	mask2load[5] = '".ICMS_URL."/modules/{$MDIR}/class/edge/masks/2bit/softedge.gif';
	</script>
	<script type='text/javascript' src='".ICMS_URL."/modules/{$MDIR}/class/edge/edge.js'></script>";
}elseif($thumbnail_mode=="slided"){
  $script="<script type='text/javascript' src='".ICMS_URL."/modules/{$MDIR}/class/slided/slided.js'></script>";
}elseif($thumbnail_mode=="instant"){
  $script="<script type='text/javascript' src='".ICMS_URL."/modules/{$MDIR}/class/instant/instant.js'></script>";
}elseif($thumbnail_mode=="shadow"){
	$script="
	<style type='text/css'>
  .blur2{
...
<script type='text/javascript' src='".ICMS_URL."/modules/{$MDIR}/class/jquery.dimensions.js'></script>
<script type='text/javascript' src='".ICMS_URL."/modules/{$MDIR}/class/jquery.dropshadow.js'></script>
<script type='text/javascript'>
